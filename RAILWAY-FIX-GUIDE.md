...
| 5 | **Benarkan IP keluar Railway:** tambah **`152.55.184.0/24`** (format CIDR) di **https://app.brevo.com/security/authorised_ips** → sahkan ia tersenarai → uji | ⭐ PENYELESAIAN MUKTAMAD (terbukti 2026-09-18): Railway **mengitar IP** dari subnet `152.55.184.x` (pernah `208.77.244.18`, `…51`, `…105`) — satu entri CIDR `/24` meliputi 256 IP, tak perlu update lagi. IP tunggal TIDAK cukup (`.51` tersenarai tapi `.105` tetap ditolak). Kalau suatu hari keluar dari subnet lain, mesej ralat papar IP baharu → tambah `/24` subnet itu |
